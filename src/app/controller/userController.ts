...
export const register = catchAsyncErrors(
  async (req: Request, res: Response, next: NextFunction) => {
    ...
    const { avatar, resume }: any = req.files;
...
export const updateProfile = catchAsyncErrors(
  async (req: Request & { user: any }, res: Response, next: NextFunction) => {
    ...
    if (req.files && req.files.avatar) {
      ...
      const user: any = await User.findById({ _id: req.user.id });
    ...
    if (req.files && req.files.resume) {
      const resume: any = req.files.resume;
...
export const updatePassword = catchAsyncErrors(
  async (req: Request & { user: any }, res: Response, next: NextFunction) => {
    ...
    const user: any = await User.findById({ _id: req.user.id }).select(
      "+password"
    );
...
export const getUserForPortfolio = catchAsyncErrors(
  async (req: Request, res: Response, next: NextFunction) => {
...
export const resetPassword = catchAsyncErrors(
  async (req: Request & { user: any }, res: Response, next: NextFunction) => {
    ...
    if (req.body.password !== req.body.confirmPassword) {
      return next(
        new ErrorHandler(401, "Password & Confirm Password do not match")
      );
    }
