...
export const addNewProject = catchAsyncErrors(
  async (req: Request, res: Response, next: NextFunction) => {
    ...
    const { projectBanner }: any = req.files;
    const {
      title,
      description,
      gitFRepoLink,
...
export const updateProject = catchAsyncErrors(
  async (req: Request, res: Response, next: NextFunction) => {
    ...
    if (req.files && req.files.projectBanner) {
      const projectBanner: any = req.files.projectBanner;
      ...
      const newProjectImage: any = await cloudinary.uploader.upload(
        projectBanner.tempFilePath,
        {
          folder: "PORTFOLIO PROJECT IMAGES",
        }
...
export const getSingleProject = catchAsyncErrors(
  async (req: Request, res: Response, next: NextFunction) => {
    const { id } = req.params;
    try {
